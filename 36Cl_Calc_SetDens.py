...
f, ax1 = plot.subplots()
# Plot and label interpolated density surface
ax1.hexbin(TexR,InhR, cmap='viridis', linewidths=0.1)
...
ax1.tick_params(axis='both', which='major', labelsize=8)
ax1.set_xlabel('Exposure Age (ka)', fontsize=8)
ax1.set_ylabel('Inheritance\n(Atoms 36Cl/g x 10^6)', fontsize=8)
BF = Line2D([0],[0], linestyle='none', marker=(5,1), markerfacecolor='red')
Mean = Line2D([0],[0], linestyle='none', marker='d', markerfacecolor='green') 
Med = Line2D([0],[0], linestyle='none', marker='o', markerfacecolor='blue')
f.legend((BF, Mean, Med), ("Best Fit", "Mean", "Median"), 'lower left', 
         numpoints=1, fontsize=6, ncol=1)
f.suptitle('Inheritance vs Age Crossplot Colored by Density', fontsize=14)
f.subplots_adjust(top=.92)
plot.savefig('f5.pdf')
plot.close('all')
...
f, ax1 = plot.subplots()
# Plot and label interpolated density surface
ax1.hexbin(InhR,EroR, cmap='viridis', linewidths=0.1)

# Plot best fit, mean and median models
ax1.plot(BestInh,BestEro, 'r', marker=(5,1)) 
ax1.plot(InhM,EroM, 'gd') 
ax1.plot(InhMed,EroMed, 'bo')
# Set Axes
ax1.set_xlim(InhR.min(),InhR.max())
ax1.set_ylim(EroR.min(),EroR.max())
ax1.tick_params(axis='both', which='major', labelsize=8)
ax1.set_xlabel('Inheritance\n(Atoms 36Cl/g x 10^6)', fontsize=8)
ax1.set_ylabel('Erosion Rate (cm/kyr)', fontsize=8)
BF = Line2D([0],[0], linestyle='none', marker=(5,1), markerfacecolor='red')
Mean = Line2D([0],[0], linestyle='none', marker='d', markerfacecolor='green') 
Med = Line2D([0],[0], linestyle='none', marker='o', markerfacecolor='blue')
f.legend((BF, Mean, Med), ("Best Fit", "Mean", "Median"), 'lower left', 
         numpoints=1, fontsize=6, ncol=1)
f.suptitle('Erosion Rate vs Inheritance Crossplot Colored by Density', fontsize=14)
f.subplots_adjust(top=.92)
plot.savefig('f6.pdf')
plot.close('all')
...
f.legend((BF, Mean, Med), ("Best Fit", "Mean", "Median"), 'lower left', 
         numpoints=1, fontsize=6, ncol=1)
f.suptitle('Inheritance vs Age Crossplot Colored by Likelihood', fontsize=14)
f.subplots_adjust(top=.92)
plot.savefig('f8.pdf')
plot.close('all')
...
for i in range(0,len(TexRPThin)):
    if LikNormPThin[i] <= LikNormMid: # Set color to plot based on likelihood
        ...
        color = [(255 - (205 * (1 - (LikNormMax - LikNormPThin[i])/
                ...
                0] #B
        size=6
    ax1.plot(InhRPThin[i], EroRPThin[i], 'o', markerfacecolor=color, 
             markeredgecolor='none', markersize=size)
...
f.legend((BF, Mean, Med), ("Best Fit", "Mean", "Median"), 'lower left', 
         numpoints=1, fontsize=6, ncol=1)
f.suptitle('Erosion vs Inheritance Crossplot Colored by Likelihood', fontsize=14)
f.subplots_adjust(top=.92)
plot.savefig('f9.pdf')
plot.close('all')
#%%
